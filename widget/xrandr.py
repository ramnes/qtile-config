...
@author ultrabug (ported to qtile by ramnes)
"""
import shlex
...
class XRandR(ThreadedPollText):
    ...
    defaults = [
        ("update_interval", 30, "Update interval in seconds, if none, the "
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.max_width = 0
        super(XRandR, self).__init__(*args, **kwargs)
    ...
    def _get_layout(self):
        ...
        for line in current.stdout.readlines():
            ...
            try:
                ...
                logger.warning('xrandr error="{}"'.format(err))
            else:
                layout[state][output] = {
                    'infos': infos,
                    'mode': mode,
    ...
    def _set_available_combinations(self):
        ...
        self.max_width = 0
        for output in range(len(self._layout['connected'])):
            for comb in combinations(self._layout['connected'], output + 1):
    ...
    def _apply(self, force=False):
        ...
        if self.displayed == self.active_layout and not force:
            ...
            logger.error("no change, do nothing")
            return
        ...
        if combination is None and mode is None:
            ...
            logger.error("displayed combination cannot be activated")
            return

        cmd = 'xrandr'
        outputs = list(self._layout['connected'].keys())
        outputs += list(self._layout['disconnected'].keys())
        ...
        logger.warning('command "{}" exit code {}'.format(cmd, code))
